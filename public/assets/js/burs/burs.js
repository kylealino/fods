var __mysys_burs_ent = new __mysys_burs_ent();
function __mysys_burs_ent() {  
	...
	this.__burs_saving = function() { 
		'use strict' 
		// Fetch all the forms we want to apply custom Bootstrap validation styles to
		var forms = document.querySelectorAll('.myburs-validation')
		...
		.forEach(function (form) {
			form.addEventListener('submit', function (event) {
				...
				try {
					...
					var burs_date = document.getElementById("burs_date");
					...
					var mparam = { 
						...
						burs_date: burs_date.value,
						meaction: 'MAIN-SAVE'
					}

					jQuery.ajax({ // default declaration of ajax parameters
						type: "POST",
						url: mesiteurl + 'myburs',
						context: document.body,
						data: eval(mparam),
						global: false,
						cache: false,
						success: function(data) {
							jQuery('.myburs-outp-msg').html(data);
							return false;
						},
